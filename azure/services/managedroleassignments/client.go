...
func newClient(auth azure.Authorizer) *azureClient {
	assignments := newRoleAssignmentClient(auth.SubscriptionID(), auth.BaseURI(), auth.Authorizer())
	definitions := newRoleDefinitionClient(auth.SubscriptionID(), auth.BaseURI(), auth.Authorizer())
	return &azureClient{assignments, definitions}
}
...
func newRoleAssignmentClient(subscriptionID string, baseURI string, authorizer autorest.Authorizer) authorization.RoleAssignmentsClient {
	client := authorization.NewRoleAssignmentsClientWithBaseURI(baseURI, subscriptionID)
...
func (ac *azureClient) CreateOrUpdateAsync(ctx context.Context, spec azure.ResourceSpecGetter, resumeToken string, parameters interface{}) (interface{}, *runtime.Poller[armauthorization.RoleAssignmentsClientCreateResponse], error) {
	...
	if err != nil {
		log.Error(err, "create role assignment failed")
	} else {
		log.Info("create role assignment successfully", "id", result.ID, "name", result.Name, "result", result)
	}
...
func (ac *azureClient) DeleteAsync(ctx context.Context, spec azure.ResourceSpecGetter, resumeToken string) (*runtime.Poller[armauthorization.RoleAssignmentsClientDeleteResponse], error) {
